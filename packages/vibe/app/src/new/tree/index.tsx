...
import { Category, Tree } from "../../types.js";
...
const Row = ({
    ...
    return (
        <div
            className={`story-row ${active ? "active" : ""}`}
            onClick={onClick ? onClick : () => {}}
        >
            <div className='story-icon-wrapper' style={{ "--fill": colorCategories[level] }}>
                {type === "folder" ? <FolderIcon /> : <Story />}
            </div>
...
export const useFilter = (data: Category[], search: string) => {
    const [filteredTree, setFilteredTree] = React.useState<Tree>(data);
...
export const Stories = ({ data, search }: { data: Category[]; search: string }) => {
    const { useLocation } = useVibeContext();
    const location = useLocation();
    const pathSegments = location.pathname.split("/");
    pathSegments.shift();
    const id = useId();

    const filteredTree = useFilter(data, search);
