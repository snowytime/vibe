...
export const Graph = () => {
    ...
    return (
        <div className='vibe__story-panel'>
...
const Row = ({
    ...
    return (
        <div
            className={`vibe__story-row ${active ? "active" : ""}`}
            onClick={onClick || (() => {})}
            role='button'
            tabIndex={0}
        >
            ...
            {/* @ts-ignore */}
            <div className='vibe__story-icon' style={{ "--fill": colorCategories[level] }}>
                {type === "folder" ? <FolderIcon /> : <StoryIcon />}
            </div>
            <div className='vibe__story-title'>{title}</div>
        </div>
    );
};
